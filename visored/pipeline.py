...
from visored.mangadex import MangaDexClient, page_url
...
async def _producer(
    ...
    for ch in manga_feed_items:
        ...
        for page_index, filename in enumerate(files):
            if str(page_index) in man.pages_done:
                rec = man.pages_done[str(page_index)]
                if rec.source_filename == str(filename):
                    pbar_pages.update(1)
                    continue
            if trust_existing:
                import glob
            ...
            url = page_url(str(base_url), quality, str(chash), str(filename))
            try:
